...
describe("Technical Center contract", () => {
  ...
  it("uses typed readiness and review workflow endpoints", async () => {
    vi.mocked(api.get).mockResolvedValue({
      data: { data: { overallReady: false, blockers: [] } },
    })
    ...
    await technicalApi.tenders.requestReview("t1", {
      type: "TECHNICAL",
      revision: 2,
    })
    await technicalApi.tenders.decideReview("t1", "rv1", {
      status: "REJECTED",
      comment: "نیازمند اصلاح",
      revision: 2,
    })
    ...
    expect(api.post).toHaveBeenCalledWith(
      "/technical/tenders/t1/reviews/rv1/decision",
      expect.objectContaining({ status: "REJECTED", comment: "نیازمند اصلاح" })
    )
  })
  ...
  it("uploads a technical document file and version in one request", async () => {
    vi.mocked(api.post).mockResolvedValue({
      data: { data: { id: "v1", version: "2.0" } },
    })
    const file = new File(["content"], "architecture.pdf", {
  ...
  it("uses qualification, dependency and requirement task endpoints", async () => {
    ...
    await technicalApi.tenders.saveQualification("t1", {
      qualificationDecision: "CONDITIONAL_GO",
      qualificationConditions: "تأیید قیمت",
    })
    await technicalApi.tenders.addDependency("t1", "r1", "r2")
    await technicalApi.tenders.linkTask("t1", "r1", "task1")
    expect(api.patch).toHaveBeenCalledWith(
      "/technical/tenders/t1/qualification",
      expect.objectContaining({ qualificationDecision: "CONDITIONAL_GO" })
    )
    expect(api.post).toHaveBeenCalledWith(
      "/technical/tenders/t1/requirements/r1/dependencies",
      { dependsOnRequirementId: "r2" }
    )
    expect(api.post).toHaveBeenCalledWith(
      "/technical/tenders/t1/requirements/r1/link-task",
      { taskId: "task1" }
    )
  })
  ...
  it("requests tender people from the selected server-side team scope", async () => {
    ...
    expect(api.get).toHaveBeenCalledWith("/users/assignee-options", {
      params: expect.objectContaining({ search: "علی", teamId: "team-1" }),
    })
  })
})
...
describe("Technical Center behavior", () => {
  it("submits the selected file from the document version dialog", async () => {
      ...
      <MemoryRouter initialEntries={["/technical/documents/d1"]}>
        <Routes>
          <Route
            ...
            element={<TechnicalDocumentDetailPage />}
          />
        </Routes>
      </MemoryRouter>,
      { wrapper }
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "نسخه جدید" })
    )
  ...
  it("renders actionable tender readiness blockers and separate reviews", async () => {
    useAuthStore.setState({
      user: { ...user, permissions: ["technical-tender:view"] },
      status: "authenticated",
    })
    const readiness = {
      overallReady: false,
      blockers: [{ code: "MANDATORY_REQUIREMENTS_INCOMPLETE", count: 2 }],
      warnings: [],
      checks: {
        ...
        commercialReview: { status: "NOT_STARTED" },
        submissionDeadline: { value: "2026-09-01", overdue: false },
        requiredTenderFields: { complete: true, missing: [] },
      },
    }
    const tender = {
      id: "t1",
    ...
    render(
      <MemoryRouter initialEntries={["/technical/tenders/t1"]}>
        <Routes>
          <Route
            path="/technical/tenders/:id"
            element={<TechnicalTenderDetailPage />}
          />
        </Routes>
      </MemoryRouter>,
      { wrapper }
    ...
    expect(
      screen.getByText(/الزامات الزامی تکمیل نشده‌اند/)
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByText(
        /در وضعیت فعلی اقدامی متناسب با دسترسی‌های شما وجود ندارد/
      )
    ).toBeInTheDocument()
  })
  it("sends document confidentiality and tender filters from the URL-backed UI", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "فیلترهای پیشرفته" })
    )
  ...
  it("requires and forwards a reason for consequential lifecycle changes", async () => {
    ...
    await userEvent.type(
      screen.getByLabelText("دلیل تغییر وضعیت"),
      "تغییر تصمیم مشتری"
    )
  ...
  it("renders one responsive card representation, pagination and accessible status", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /نسخه پایدار/ })
    ).toBeInTheDocument()
