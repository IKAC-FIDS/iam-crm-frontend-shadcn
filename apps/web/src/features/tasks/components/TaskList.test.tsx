...
describe("TaskList", () => {
  it("renders the unified card with all permitted actions visible", async () => {
    ...
    expect(screen.getByText("واگذاری مستقیم به این کاربر")).toBeInTheDocument()
    expect(screen.queryByText("شخصی")).not.toBeInTheDocument()
    expect(screen.queryByText("۰ از ۰")).not.toBeInTheDocument()
