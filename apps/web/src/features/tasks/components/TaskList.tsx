  ...
  const fields: EntityCardField<Task>[] = [
    {
      ...
      render: (task) => {
        ...
        return <span className="inline-flex min-w-0 items-center gap-2">
          <IdentityAvatar name={companyName || taskContextLabel(task)} mediaPath={task.company?.id ? `/companies/${task.company.id}/logo` : null} hasMedia={Boolean(task.company?.logoObjectKey)} mediaVersion={task.company?.logoObjectKey} fallbackIcon={<Building2 className="size-4" />} className="size-8 rounded-xl text-[10px]" />
    ...
    },
    {
      ...
      render: (task) => {
        ...
        return <span className="inline-flex min-w-0 items-center gap-2">
          <IdentityAvatar name={name} mediaPath={task.assignedTo?.id ? `/users/${task.assignedTo.id}/avatar` : null} hasMedia={Boolean(task.assignedTo?.avatarObjectKey)} mediaVersion={task.assignedTo?.avatarObjectKey} className="size-8 rounded-xl text-[10px]" />
