...
export function TaskActionDialogs({
  ...
  if (action === "delete")
    return <DeleteDialog task={task} onClose={onClose} onDeleted={onDeleted} />
  if (action === "complete")
    return <CompleteDialog task={task} onClose={onClose} />
  ...
  if (action === "subtask")
    return <SubtaskDialog task={task} onClose={onClose} />
...
function CompleteDialog({
  ...
  return (
    ...
    >
      {incompleteCount ? (
        <div className="rounded-xl border border-[var(--destructive)]/25 bg-[var(--destructive-soft)] p-3 text-xs text-[var(--destructive)]">
          {incompleteCount.toLocaleString("fa-IR")} زیرکار ناتمام باقی مانده
          است. ابتدا زیرکارها را بررسی کنید.
        </div>
      ) : null}
...
function AssignDialog({ task, onClose }: { task: Task; onClose: () => void }) {
  ...
  const [scope, setScope] = useState<TaskAssignmentScope>(
    task.assignmentScope || "SELF"
  )
  const [teamSearch, setTeamSearch] = useState("")
  const [team, setTeam] = useState<TaskOption | undefined>(
    task.team
  ...
  const teamOptions = useMemo(
    () => teamQuery.data?.pages.flatMap((page) => page.data) || [],
    [teamQuery.data]
  )
  async function submit() {
    ...
    try {
      await mutation.mutateAsync({
        ...
        payload: {
          ...
          assigneeId: scope === "SELF" ? undefined : assignee?.id,
  ...
  return (
    <SimpleDialog
      ...
      onSubmit={submit}
    >
      <div className="rounded-xl border p-3 text-xs text-[var(--app-text-secondary)]">
        وضعیت فعلی: {task.assignedTo?.fullName || text.labels.unassigned} ·{" "}
        {task.team?.name || "بدون تیم"}
      </div>
      ...
      {scope === "TEAM" ? (
        <Field label="تیم">
          <TaskOptionSelect
            ...
            loadingMore={teamQuery.isFetchingNextPage}
            onLoadMore={() => void teamQuery.fetchNextPage()}
          />
        </Field>
      ) : null}
      {scope !== "SELF" ? (
    ...
    () =>
      assignees.data?.pages
        ...
        .map((item) => ({
          id: item.id,
          label: item.fullName || item.email || item.id,
          secondary: [item.email, item.role, item.teamRef?.name || item.team]
            .filter(Boolean)
            .join(" · "),
        })) || [],
    [assignees.data]
    ...
    try {
      await mutation.mutateAsync({
        ...
        payload: {
          ...
          assignmentScope: canAssignOthers ? scope : "SELF",
          teamId: canAssignOthers && scope === "TEAM" ? team?.id : undefined,
          assigneeId:
            canAssignOthers && scope !== "SELF" ? assignee?.id : undefined,
          inheritLinkedEntity,
        },
      })
      ...
      toast.error(getApiErrorMessage(error, uiText.tasks.errors.mutation))
    }
  }
  return (
    <SimpleDialog
      title="ایجاد زیرکار"
      description="یک کار جدید ساخته می‌شود و مسئولیت کار والد تغییر نمی‌کند."
      pending={mutation.isPending}
      submitLabel="ایجاد زیرکار"
      submitDisabled={
        !title.trim() || (canAssignOthers && scope === "TEAM" && !team)
      }
      onClose={onClose}
      onSubmit={submit}
    >
      <Field label="عنوان زیرکار">
        <input
          value={title}
      ...
      <Field label="اولویت">
        <StaticOptionSelect
          ...
          options={(
            ...
            id: value,
            label: uiText.tasks.priorities[value],
          }))}
        />
      </Field>
      <Field label="موعد انجام">
      ...
      {canAssignOthers ? (
          ...
          {scope !== "SELF" ? (
            <Field label="مسئول">
              <TaskOptionSelect
                ...
                placeholder="انتخاب مسئول (اختیاری)"
        ...
        <p className="rounded-xl bg-[var(--app-primary-soft)] p-3 text-xs leading-6 text-[var(--app-primary)]">
          این زیرکار به خود شما واگذار می‌شود. برای ارجاع به سایر کاربران یا
          تیم‌ها، دسترسی <code dir="ltr">task:assign</code> لازم است.
        </p>
      )}
      <label className="flex items-center gap-2 text-xs">
        <input
          type="checkbox"
          checked={inheritLinkedEntity}
          onChange={(event) => setInheritLinkedEntity(event.target.checked)}
        />
        ارتباط تجاری/فنی از کار والد کپی شود
      </label>
    </SimpleDialog>
  )
}
    ...
    >
      ...
      {invalidReminder ? (
        <p className="text-xs text-[var(--destructive)]">
          {text.validation.reminderBeforeDue}
        </p>
      ) : null}
    </SimpleDialog>
  )
