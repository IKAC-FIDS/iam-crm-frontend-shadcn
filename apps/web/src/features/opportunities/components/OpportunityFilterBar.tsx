...
const selectClass = "h-11 w-full rounded-xl border border-input bg-transparent px-3 text-xs outline-none focus:border-[var(--app-primary)]"
...
export function OpportunityFilterBar({
  ...
  const activeAdvanced = [filters.team, filters.ownerId, filters.stageId, filters.sourceOptionId, filters.primaryContactId, filters.archiveState !== "active"].filter(Boolean).length

  return (
    <section className="rounded-[24px] border border-[var(--app-divider)] bg-[var(--app-surface)] p-3 shadow-[var(--app-shadow-card)]">
      <div className="grid gap-3 xl:grid-cols-[minmax(220px,1.35fr)_auto_minmax(190px,1fr)_minmax(150px,.7fr)_minmax(190px,1fr)_auto] xl:items-center">
        ...
        <SearchableCompanySelect value={filters.companyId} onChange={(companyId) => onChange({ companyId, primaryContactId: undefined })} />

        <select className={selectClass} value={filters.priority ?? ""} onChange={(event) => onChange({ priority: (event.target.value || undefined) as OpportunityFilters["priority"] })}>
          <option value="">{text.filters.priority}: {text.filters.allOptions}</option>
        ...
        <PersianDateRangePicker
          value={dateRange}
          onChange={(range) => onChange({ expectedCloseFrom: toApiDate(range?.from) ?? undefined, expectedCloseTo: toApiDate(range?.to) ?? undefined })}
        ...
        <Popover>
          ...
          <PopoverContent align="end" className="w-[min(680px,calc(100vw-24px))] rounded-2xl p-4" dir="rtl">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <select className={selectClass} value={filters.ownershipScope} onChange={(event) => onChange({ ownershipScope: event.target.value as OpportunityFilters["ownershipScope"] })}>
              ...
              <Input value={filters.team ?? ""} onChange={(event) => onChange({ team: event.target.value || undefined })} placeholder={text.filters.teamPlaceholder} className="h-11 rounded-xl" />
              <select className={selectClass} value={filters.ownerId ?? ""} onChange={(event) => onChange({ ownerId: event.target.value || undefined })}>
                <option value="">{text.filters.owner}: {text.filters.allOptions}</option>
                {owners.map((owner) => <option key={owner.id} value={owner.id}>{owner.fullName}</option>)}
              </select>
              <select className={selectClass} value={filters.stageId ?? ""} onChange={(event) => onChange({ stageId: event.target.value || undefined })}>
                <option value="">{text.filters.stage}: {text.filters.allOptions}</option>
                {stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.label}</option>)}
              </select>
              <select className={selectClass} value={filters.sourceOptionId ?? ""} onChange={(event) => onChange({ sourceOptionId: event.target.value || undefined })}>
                <option value="">{text.filters.source}: {text.filters.allOptions}</option>
                {sources.map((source) => <option key={source.id} value={source.id}>{source.label}</option>)}
              </select>
              <select className={selectClass} disabled={!filters.companyId || people.isLoading || people.isError} value={filters.primaryContactId ?? ""} onChange={(event) => onChange({ primaryContactId: event.target.value || undefined })}>
                <option value="">{text.filters.primaryContact}: {text.filters.allOptions}</option>
                {contacts.map((person) => <option key={person.id} value={person.id}>{person.fullName}</option>)}
              </select>
              <select className={selectClass} value={filters.archiveState} onChange={(event) => onChange({ archiveState: event.target.value as OpportunityFilters["archiveState"] })}>
      ...
      <div className="mt-3 flex justify-end">
        <Button type="button" variant="ghost" size="sm" className="rounded-xl text-[var(--app-text-secondary)]" onClick={onClear}>
