...
describe('Timesheet screens', () => {
  ...
  it('sends report employee and type filters to the server', async () => { setup(<TimesheetReportsPage />); await screen.findByRole('option', { name: 'تیم نمونه' }); fireEvent.change(screen.getByLabelText('تیم'), { target: { value: 't' } }); fireEvent.change(screen.getByLabelText('نوع کار'), { target: { value: 'OVERTIME' } }); await waitFor(() => expect(api.get).toHaveBeenCalledWith('/admin/timesheets/reports', { params: expect.objectContaining({ teamId: 't', entryType: 'OVERTIME' }) })) })
})
