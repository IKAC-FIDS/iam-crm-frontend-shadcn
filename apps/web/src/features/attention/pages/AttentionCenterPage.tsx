      ...
      <section className="relative overflow-hidden rounded-[30px] border border-[var(--app-divider)] bg-[var(--app-surface)] px-5 py-6 shadow-[var(--app-shadow-card)] sm:px-7">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex rounded-xl border border-[var(--app-divider)] bg-[var(--app-background)] p-1">
            ...
            {canNotifications ? <Button type="button" variant="ghost" size="sm" className={tab === "notifications" ? "rounded-lg bg-[var(--app-surface)] text-[var(--app-primary)] shadow-sm" : "rounded-lg"} onClick={() => patch({tab:"notifications",page:"1",quick:null})}><Bell className="size-4" />اعلان‌ها{stats.unread > 0 ? <span className="rounded-full bg-[var(--destructive)] px-1.5 text-xs text-white">{stats.unread.toLocaleString("fa-IR")}</span> : null}</Button> : null}
          </div>
        </div>
      </section>
...
function FollowUpList({items,loading,error,fetching,page,pageCount,filter,canComplete,canReschedule,onRetry,onPage,onFilter,onCompany}:{items:FollowUpActivity[];loading:boolean;error:boolean;fetching:boolean;page:number;pageCount:number;filter:FollowUpFilter;canComplete:boolean;canReschedule:boolean;onRetry:()=>void;onPage:(page:number)=>void;onFilter:(value:FollowUpFilter)=>void;onCompany:(id:string)=>void}) {
  ...
  const columns:DataTableColumn<FollowUpActivity>[] = [
    {id:"followUp",header:"پیگیری",className:"min-w-60",cell:(item)=><div className="min-w-0"><p className="truncate text-xs font-bold text-[var(--app-heading)]">{item.outcome || activityLabel(item.type)}</p><p className="mt-1 line-clamp-1 text-xs text-[var(--app-text-secondary)]">{item.notes || "بدون یادداشت"}</p></div>},
...
function NotificationList({canManage,navigate}:{canManage:boolean;navigate:(path:string)=>void}) {
  ...
  const columns:DataTableColumn<Notification>[]=[
    {id:"notification",header:"اعلان",className:"min-w-72",cell:(n)=><div className="min-w-0"><div className="flex items-center gap-2">{!n.readAt?<span className="size-2 shrink-0 rounded-full bg-[var(--app-primary)]"/>:null}<p className="truncate text-xs font-bold text-[var(--app-heading)]">{n.title}</p></div><p className="mt-1 line-clamp-1 text-xs text-[var(--app-text-secondary)]">{n.body||"بدون توضیح"}</p></div>},
