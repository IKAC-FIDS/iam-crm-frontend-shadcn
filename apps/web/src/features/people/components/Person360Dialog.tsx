...
function InfoGrid({ items }: { items: Array<[string, string | null | undefined]> }) { return <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">{items.map(([label, value]) => <div key={label} className="rounded-2xl bg-[var(--app-background)]/60 p-3"><p className="text-xs font-bold text-[var(--app-text-secondary)]">{label}</p><p className="mt-1.5 text-xs text-[var(--app-heading)]">{value || uiText.people.notSpecified}</p></div>)}</div> }
function RoleBadge({ label, primary = false }: { label: string; primary?: boolean }) { return <span className={primary ? "inline-flex items-center gap-1 rounded-full bg-[var(--app-primary-soft)] px-2.5 py-1 text-xs font-bold text-[var(--app-primary)]" : "inline-flex items-center gap-1 rounded-full border border-[var(--app-outline)] bg-[var(--app-surface)] px-2.5 py-1 text-xs font-bold text-[var(--app-primary-alt)]"}><Star className={primary ? "size-3 fill-current" : "size-3"} />{label}</span> }
function IntelligenceBadge({ label }: { label: string }) { return <span className="rounded-full border border-[var(--app-primary)]/15 bg-[var(--app-surface)]/75 px-2.5 py-1 text-xs font-bold text-[var(--app-on-primary-container)]">{label}</span> }
