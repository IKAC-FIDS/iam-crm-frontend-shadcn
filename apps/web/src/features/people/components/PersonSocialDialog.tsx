...
export function PersonSocialDialog({ open, onOpenChange, social, isPending, error, onSubmit }: {
  ...
  const lookup = usePeopleLookup("social_types")
  const options = Array.isArray(lookup.data) ? lookup.data : []
  const [platformOptionId, setPlatformOptionId] = useState("")
  const [handle, setHandle] = useState("")
  const [note, setNote] = useState("")
  const [isPrimary, setIsPrimary] = useState(false)
  useEffect(() => { if (open) { setPlatformOptionId(social?.platformOptionId || social?.platformOption?.id || ""); setHandle(social?.handle || ""); setNote(social?.note || ""); setIsPrimary(Boolean(social?.isPrimary)) } }, [open, social])
  async function submit(event: React.FormEvent) { event.preventDefault(); if (platformOptionId && handle.trim()) await onSubmit({ platformOptionId, handle: handle.trim(), note: note.trim() || undefined, isPrimary }) }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent showCloseButton={false} dir="rtl" className="max-w-lg gap-0 rounded-[26px] border-[var(--app-divider)] bg-[var(--app-surface)] p-0"><DialogHeader className="flex-row items-center justify-between border-b border-[var(--app-divider)] p-5"><DialogTitle>{social ? text.edit : text.add}</DialogTitle><Button type="button" variant="ghost" size="icon" className="rounded-xl" aria-label={uiText.people.actions.close} onClick={() => onOpenChange(false)}><X className="size-4" /></Button></DialogHeader><form onSubmit={submit} className="grid gap-4 p-5">
    <Field label={text.platform}><select value={platformOptionId} onChange={(event) => setPlatformOptionId(event.target.value)} className="h-11 rounded-xl border border-input bg-background px-3 text-sm"><option value="">{text.selectPlatform}</option>{options.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></Field>
    ...
    <div className="flex justify-end gap-2 border-t border-[var(--app-divider)] pt-4"><Button type="button" variant="outline" className="rounded-xl" onClick={() => onOpenChange(false)}>{uiText.common.cancel}</Button><Button type="submit" disabled={isPending || !platformOptionId || !handle.trim()} className="rounded-xl bg-[var(--app-primary)] hover:bg-[var(--app-primary-hover)]">{isPending ? uiText.common.processing : uiText.people.actions.save}</Button></div>
