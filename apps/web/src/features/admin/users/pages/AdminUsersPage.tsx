...
export function AdminUsersPage() {
  ...
  return <div className="grid gap-5" dir="rtl">
    <section className="relative overflow-hidden rounded-[30px] border border-[var(--app-divider)] bg-[var(--app-surface)] px-5 py-6 shadow-[var(--app-shadow-card)] sm:px-7"><div className="pointer-events-none absolute -end-16 -top-20 size-64 rounded-full bg-[var(--app-primary-soft)] blur-3xl" /><div className="relative flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between"><div><div className="ui-eyebrow mb-3 inline-flex items-center gap-2"><Sparkles className="size-4" />مرکز مدیریت کاربران</div><h1 className="ui-page-title">مدیریت کاربران</h1><p className="mt-2 max-w-3xl text-sm leading-7 text-muted-foreground">کاربران، نقش‌ها، تیم‌ها، وضعیت دسترسی و تاریخچه تغییرات را از یک فضای واحد مدیریت کنید.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => void refresh()}><RefreshCcw className="ms-2 size-4" />به‌روزرسانی</Button>{canCreate ? <Button onClick={() => setCreateOpen(true)}><Plus className="ms-2 size-4" />افزودن کاربر</Button> : null}</div></div></section>
