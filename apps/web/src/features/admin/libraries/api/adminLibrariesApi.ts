...
export const lookupGroups = [
  ...
  ["job-titles", "سمت‌ها"],
  ["seniority-levels", "سطح ارشدیت"],
  ["persona-roles", "نقش‌های فروش"],
  ["opportunity-sources", "منابع ایجاد فرصت"],
  ["activity-types", "انواع فعالیت"],
  ["person-social-platforms", "شبکه‌های اجتماعی"],
  ["company-sources", "منابع شرکت"],
...
const endpoints: Record<LibraryKind, string> = {
...
const endpoint = (kind: LibraryKind, group?: LookupGroup) =>
  kind === "lookupOptions" ? `${endpoints[kind]}/${group}` : endpoints[kind]
...
const normalize = (input: unknown): LibraryItem => {
  ...
  return {
    id: String(raw.id),
    label: String(
      raw.name ?? raw.title ?? raw.titlePattern ?? raw.label ?? raw.code ?? ""
    ),
    description:
    ...
    defaultPainPoint:
      raw.defaultPainPoint == null ? null : String(raw.defaultPainPoint),
    defaultUseCase:
      raw.defaultUseCase == null ? null : String(raw.defaultUseCase),
    raw,
  }
}
const body = (kind: LibraryKind, value: LibraryPayload) => {
  if (kind === "industries")
    return { name: value.primary, description: value.description || undefined }
  ...
  if (kind === "painPoints" || kind === "useCases")
    return {
      title: value.primary,
      description: value.description || undefined,
  ...
  if (kind === "personas")
    return {
      titlePattern: value.primary,
      defaultPainPoint: value.defaultPainPoint || undefined,
      defaultUseCase: value.defaultUseCase || undefined,
...
export async function getLibraryItems(kind: LibraryKind, group?: LookupGroup) {
  if (kind === "leadSources" || kind === "lookupOptions") {
    const [active, inactive] = await Promise.all([
      fetchState(kind, group, true),
      fetchState(kind, group, false),
    ])
...
export async function saveLibraryItem(
  ...
  const response = id
    ? await api.patch(`${endpoint(kind, group)}/${id}`, body(kind, payload))
    : await api.post(endpoint(kind, group), body(kind, payload))
...
export async function removeLibraryItem(
  ...
  await api.delete(`${endpoint(kind, group)}/${id}`)
}
  ...
  return parsePaginatedResponse(
    ...
    z.custom<Product>(
      (value) =>
        ...
        "type" in value &&
        (value.type === "HARDWARE" || value.type === "SOFTWARE")
    )
  )
}
export async function saveProduct(payload: ProductPayload, id?: string) {
  const body = {
    ...payload,
    digikalaCode: payload.digikalaCode?.trim() || null,
    digikalaUrl: payload.digikalaUrl?.trim() || null,
  }
  const response = id
    ? await api.patch(`/product-catalog/${id}`, body)
    : await api.post("/product-catalog", body)
  return unwrapApiResponse<Product>(response.data)
}
export async function toggleProduct(item: Product) {
  const response = await api.patch(
    `/product-catalog/${item.id}/${item.isActive ? "deactivate" : "activate"}`
  )
  return unwrapApiResponse<Product>(response.data)
}
export async function getProductPriceHistory(id: string) {
  const response = await api.get(`/product-catalog/${id}/price-history`, {
    params: { page: 1, limit: 50 },
  })
  return unwrapApiResponse<{ data: ProductPriceHistory[]; meta: PageMeta }>(
    response.data
  )
}
