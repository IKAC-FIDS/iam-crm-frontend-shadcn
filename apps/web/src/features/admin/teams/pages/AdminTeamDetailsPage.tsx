...
export function AdminTeamDetailsPage() {
  ...
  const usersQuery = useQuery({
    queryKey: ["admin-team-all-users"],
    queryFn: getAllUsers,
  })

  const auditQuery = useQuery({
    queryKey: ["admin-team-audit", teamId],
    queryFn: () => getTeamAuditLogs(teamId as string),
    enabled: Boolean(teamId) && canViewAudit,
  })
  ...
  const refreshTeam = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ["admin-team-members", teamId] })
    await queryClient.invalidateQueries({ queryKey: ["admin-teams"] })
    await queryClient.invalidateQueries({ queryKey: ["admin-team-audit", teamId] })
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "ویرایش تیم انجام نشد.")),
  })
  ...
  const statusMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "تغییر وضعیت تیم انجام نشد.")),
  })
  ...
  const addMemberMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "افزودن عضو انجام نشد.")),
  })
  ...
  const removeMemberMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "حذف عضو از تیم انجام نشد.")),
  })

  if (teamQuery.isLoading) {
    return <div className="grid min-h-72 place-items-center text-sm text-muted-foreground">در حال دریافت اطلاعات تیم...</div>
  }
    ...
    return (
      <div className="grid min-h-72 place-items-center text-center">
        <div>
          ...
          <Button className="mt-3" variant="outline" onClick={() => navigate("/admin/teams")}>بازگشت</Button>
        </div>
      </div>
    )
  }
    ...
    <div className="grid gap-5" dir="rtl">
      <section className="rounded-[28px] border border-[var(--app-divider)] bg-[var(--app-surface)] p-6 shadow-[var(--app-shadow-card)]">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div className="flex items-start gap-4">
            <Button size="icon-sm" variant="ghost" onClick={() => navigate("/admin/teams")}>
            ...
            <div className="grid size-16 place-items-center rounded-[22px] bg-[var(--app-primary-soft)] text-xl font-black text-[var(--app-primary)]">
              {team.name.split(/\s+/).slice(0, 2).map((item) => item[0]).join("")}
            </div>
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground" dir="ltr">{team.code}</p>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-muted-foreground">{team.description || "توضیحی برای این تیم ثبت نشده است."}</p>
            </div>
          </div>
          ...
          {canManage ? (
            ...
            >
              {team.isActive ? <Ban className="ms-2 size-4" /> : <BadgeCheck className="ms-2 size-4" />}
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <article className="rounded-[22px] border border-[var(--app-divider)] bg-[var(--app-surface)] p-4">
          ...
          <div className="mt-2 truncate font-black">{team.manager?.fullName || "بدون مدیر"}</div>
        </article>
      </section>
        ...
        <article className="rounded-[24px] border border-[var(--app-divider)] bg-[var(--app-surface)] p-5 shadow-[var(--app-shadow-card)]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input value={code} onChange={(event) => setCode(event.target.value)} dir="ltr" disabled={!canManage} />
            </div>
            <div className="sm:col-span-2">
              <label className="mb-1.5 block text-xs font-bold text-muted-foreground">مدیر تیم</label>
              <NativeSelect value={managerId} onChange={(event) => setManagerId(event.target.value)} disabled={!canManage}>
          ...
          {canManage ? (
            <>
              <p className="mb-4 text-xs leading-6 text-muted-foreground">
                افزودن کاربری که عضو تیم دیگری است، او را به این تیم منتقل می‌کند.
              </p>
              <div className="flex gap-2">
                <NativeSelect value={newMemberId} onChange={(event) => setNewMemberId(event.target.value)} disabled={!team.isActive}>
                ...
                <Button
                  ...
                  disabled={!newMemberId || !team.isActive || addMemberMutation.isPending}
                >
                  <Plus className="size-4" />
                </Button>
              </div>
              {!team.isActive ? (
                <p className="mt-3 text-xs text-amber-700">برای افزودن عضو، ابتدا تیم را فعال کنید.</p>
              ) : null}
            </>
          ) : (
            <p className="text-sm text-muted-foreground">مجوز مدیریت تیم برای افزودن عضو لازم است.</p>
          )}
        </article>
      </section>
      ...
      <section className="overflow-hidden rounded-[24px] border border-[var(--app-divider)] bg-[var(--app-surface)] shadow-[var(--app-shadow-card)]">
        <div className="border-b border-[var(--app-divider)] px-5 py-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">{fa(members.length)} عضو</p>
        </div>

        {!members.length ? (
          <div className="grid min-h-48 place-items-center text-sm text-muted-foreground">عضوی در این تیم وجود ندارد.</div>
        ) : (
              ...
              <tbody>
                {visibleMembers.map((member) => (
                  <tr key={member.id} className="border-t border-[var(--app-divider)] hover:bg-muted/25">
                    ...
                    <td className="px-4 py-4">{member.isActive ? "فعال" : "غیرفعال"}</td>
                    <td className="px-4 py-4 text-center">
                      {canManage ? (
                        <Button size="icon-sm" variant="ghost" onClick={() => removeMemberMutation.mutate(member.id)} aria-label="حذف از تیم">
      ...
      <section className="rounded-[24px] border border-[var(--app-divider)] bg-[var(--app-surface)] p-5 shadow-[var(--app-shadow-card)]">
        ...
        {!canViewAudit ? (
          <p className="text-sm text-muted-foreground">مجوز مشاهده Audit Log برای این بخش لازم است.</p>
        ) : auditQuery.isLoading ? (
          <p className="text-sm text-muted-foreground">در حال دریافت تاریخچه...</p>
        ) : !(auditQuery.data ?? []).length ? (
          <p className="text-sm text-muted-foreground">رویدادی برای این تیم ثبت نشده است.</p>
        ) : (
          <div className="grid gap-3">
            {(auditQuery.data ?? []).map((item) => (
              <div key={item.id} className="rounded-2xl bg-muted/30 p-4">
                ...
                <div className="mt-1 text-xs text-muted-foreground">{dateTime(item.createdAt)}</div>
              </div>
            ))}
          </div>
        )}
